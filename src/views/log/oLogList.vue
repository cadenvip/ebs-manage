...
import { getAllOLogs, getOLogList } from '@/api/log'
...
export default {
  ...
  methods: {
    queryOLogList() {
      this.loading = true
      getOLogList(this.searchForm, this.currentPage, this.pagesize).then(response => {
    ...
    initOLogList() {
      this.loading = true
      debugger
      getAllOLogs(this.currentPage, this.pagesize).then(response => {
    ...
    detail(olog) {
      this.$router.push({ path: '/system/log/olog/detail', query: { id: olog.id }})
    },
