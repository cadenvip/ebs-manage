        ...
        <div>
          <el-form ref="registerForm" :model="registerForm" :rules="registerRules" label-width="170px" style="margin:6px">
            ...
            <el-form-item label="售后处理点：" prop="sellAddressListForm">  
              <el-row>
                <el-col :span="20">
                  <!-- <AddressSelector :locationId="locationcode" :detailAddress="selladdress" @addressChanged="getAddressInfo"></AddressSelector> -->
                  hahahaha
                </el-col>
                <el-col :span="4">
              ...
              <div v-for="(item,index) in sellAddressListForm">
                <el-row>
                  <el-col :span="20">
...
<script>
  ...
  import { auditBusinesses } from '@/api/businesses'
  ...
  export default {
    ...
    methods: {
      ...
      audit(type) {
        ...
        auditBusinesses({ 'businessesid': this.businessesid, 'state': this.registerForm.state, 'reason': this.reason }).then(response => {
          alert('审核成功')
        }).catch(error => {
          console.log(error)
        })
